...
function getRequiredFieldsByTableIndex(tableIndex: string): string[] {
  const fieldMap: { [key: string]: string[] } = {
    ...
    '9': ['routeName', 'driverIdAssigned', 'vehicleIdAssigned', 'startLocation', 'endLocation', 'estimatedDuration', 'totalDistance', 'maxCapacity', 'deliveryDays', 'startTime', 'activeFlag', 'createdBy'],
      ...
      for (const chunk of chunks) {
        ...
        for (const item of chunk) {
          ...
          const processedItem = {
            ...
            pk: `${resourceName.toUpperCase()}_${item[primaryKey] || randomUUID()}`,
    ...
    switch (method) {
      case 'GET':
        ...
        if (resourceId) {
          const result = await docClient.send(new GetCommand({
            TableName: TABLE_NAME,
            Key: {
              pk: `${resourceName.toUpperCase()}_${resourceId}`,
              sk: resourceId
            }
          }));
          ...
          const result = await docClient.send(new ScanCommand({
            ...
            ExpressionAttributeValues: {
              ':pkPrefix': `${resourceName.toUpperCase()}_`
            },
            Limit: limit
          }));
      ...
      case 'POST':
        ...
        const newItem = {
          ...createBody,
          [primaryKey]: newId,
          pk: `${resourceName.toUpperCase()}_${newId}`,
          sk: newId,
          createdAt: now,
          updatedAt: now,
          createdBy: user.id,
        ...
        await createAuditLog(user, 'CREATE', resourceName, { itemId: newId });
      ...
      case 'PUT':
        ...
        await createAuditLog(user, 'UPDATE', resourceName, { itemId: resourceId });
      ...
      case 'DELETE':
        ...
        await createAuditLog(user, 'DELETE', resourceName, { itemId: resourceId });
