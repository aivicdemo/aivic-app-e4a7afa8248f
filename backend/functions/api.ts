...
function getRequiredFieldsByEntityType(entityType: string): string[] {
  const fieldMap: { [key: string]: string[] } = {
    'STORE': ['storeCode', 'storeName', 'storeCategory', 'prefecture', 'city', 'transactionStartDate', 'transactionStatus', 'activeFlag', 'createdBy', 'updatedBy'],
    ...
    'FORECAST_REPORT': ['storeId', 'productCategory', 'forecastPeriodStart', 'forecastPeriodEnd', 'forecastDemand', 'confidenceLevel', 'seasonalFactorFlag', 'eventFactorFlag', 'recommendedPurchaseAmount', 'createdBy'],
    'MONTHLY_AGGREGATE': ['storeId', 'aggregateYearMonth', 'productCategory', 'newKeepBottles', 'completedBottles', 'totalConsumption', 'totalVisitors', 'activeMembers', 'averageConsumption', 'createdBy'],
    'SEASONAL_ANALYSIS': ['analysisYear', 'analysisMonth', 'alcoholCategory', 'regionCode', 'baseConsumption', 'actualConsumption', 'seasonalIndex', 'eventImpactFlag', 'createdBy'],
    ...
    'DELIVERY_ROUTE': ['routeName', 'driverId', 'vehicleId', 'startLocation', 'endLocation', 'estimatedDuration', 'totalDistance', 'maxCapacity', 'deliveryDays', 'startTime', 'activeFlag', 'createdBy'],
      ...
      for (const chunk of chunks) {
        ...
        for (const item of chunk) {
          ...
          const processedItem = {
            ...
            pk: `${tableConfig.entityType}#${item[tableConfig.pk] || randomUUID()}`,
    ...
    switch (method) {
      case 'GET':
        ...
        if (itemId) {
          const result = await docClient.send(new GetCommand({
            TableName: TABLE_NAME,
            Key: {
              pk: `${tableConfig.entityType}#${itemId}`,
              sk: itemId
            }
          }));
      ...
      case 'POST':
        ...
        const newItem = {
          ...
          pk: `${tableConfig.entityType}#${newId}`,
          sk: newId,
          entityType: tableConfig.entityType,
          createdAt: now,
          updatedAt: now,
