...
function getRequiredFieldsByTableIndex(tableIndex: string): string[] {
  const fieldMap: { [key: string]: string[] } = {
    ...
    '4': ['storeId', 'productCategory', 'forecastPeriodStart', 'forecastPeriodEnd', 'forecastDemand', 'confidenceLevel', 'seasonalFlag', 'eventFlag', 'recommendedStock', 'creator'],
    '5': ['storeId', 'summaryMonth', 'productCategory', 'newKeepBottles', 'completedBottles', 'totalConsumption', 'totalVisitors', 'activeMembers', 'averageConsumption', 'creator'],
...
export const handler = async (event: APIGatewayEvent): Promise<APIGatewayResponse> => {
  try {
    ...
    const itemId = pathParts[1] && pathParts[1] !== 'bulk' ? pathParts[1] : null;
      ...
      for (let i = 0; i < items.length; i += 25) {
        ...
        for (const item of batch) {
          ...
          if (validationErrors.length > 0) {
            failed++;
            errors.push(`Item ${imported + failed}: ${validationErrors.join(', ')}`);
            continue;
          }
